						...
						if (props.getArtifactType().equalsIgnoreCase(mlpArtifact.getArtifactTypeCode())) { //get TGIF file
							try {
								...
								if(null != byteArrayOutputStream && !byteArrayOutputStream.toString().isEmpty()){
									...
									// Workaround: Empty descriptions are sometimes wrongly encoded
									String baosString = byteArrayOutputStream.toString();
									baosString = baosString.replace("\"description\":{}", "\"description\":\"\"");
									tgif = mapper.readValue(baosString, Tgif.class);
								}
								if(null != tgif){
										...
										for (Provide provide : inputs) {
											// For every provide generate the keyVO
												if (null != provide.getRequest().getFormat()) {
													...
													for (MessageBody msgBody : messages) { // Assumption : only input message parameter
														if (null != msgBody.getMessageargumentList()) {															
															String msgName = msgBody.getMessageName();
															if (confprops.getModelMatchingExcludedMessages().stream().anyMatch(msgName::equalsIgnoreCase)) {
																// Skip message because it is excluded from model matching in the application properties
																continue;
															}
