...
'use strict';
angular
    .module('acuCompose',['ui.bootstrap'])
    .component(
        'acuCompose',
        {
            templateUrl : './app/acu-compose/acu-compose.template.html',
            controller : ACController
        }
    );
ACController.$inject = ['$scope','$http','$filter','$q','$window','$rootScope','$mdDialog','$state','$stateParams','$injector','browserStorageService','apiService'];

function ACController($scope,$http,$filter,$q,$window,$rootScope,$mdDialog ,$state,$stateParams,$injector, browserStorageService, apiService) {
    ...
    $scope.getDeployMenu = function(){
        console.log("getDeployMenu");
        return ui_system_config.deploy_menu;
    }

    $scope.triggerDeployment = function(menuEntry){
        console.log("triggerDeployment of "+menuEntry.title);
        if(menuEntry.produces_download) {
            $window.location.assign(menuEntry.local_service_path + $scope.solutionIdDeploy + "/" + $scope.revisionId+"?jwtToken="+$scope.auth);
        } else {
            $window.open(menuEntry.local_service_path + $scope.solutionIdDeploy + "/" + $scope.revisionId+"?jwtToken="+$scope.auth, "_blank");
        }
    }
	        ...
	        if(entry.toolKit === 'CP' || entry.toolKit === 'DS') {
	            ...
	            $http.get(url)
	                .success(function(result) {  
	                    ...
	                    $scope.revisionId = cdump.revisionId;
    ...
    var _ionicons = {
        AlarmGenerator: 'app/acu-compose/img/ios-bell.svg',
        Classifier: 'app/acu-compose/img/stats-bars.svg',
        Aggregator: 'app/acu-compose/img/network.svg',
        Predictor: 'app/acu-compose/img/android-bulb.svg',
        Recommender: 'app/acu-compose/img/thumbsup.svg',
        DataMapper: 'app/acu-compose/img/shuffle.svg',
        DS: 'app/acu-compose/img/soup-can.svg',
        Training: 'app/acu-compose/img/loop.svg',
        Others: 'app/acu-compose/img/aperture.svg',
        Images: 'app/acu-compose/img/images.svg'
